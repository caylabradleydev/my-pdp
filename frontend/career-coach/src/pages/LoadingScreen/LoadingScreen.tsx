  ...
  const fetchQuote = async () => {
    try {
      const response = await fetch(
        "https://mypdp-cta4i5qpaq-uc.a.run.app/quote/"
      );
    ...
    if (!loading && roadmap) {
      const emptyFields =
        ...
        roadmap.roadmap.some(
          (course: RoadmapSectionType) => course.courses.length == 0
        );
      if (emptyFields) {
        navigate("/error");
      } else {
  ...
  return (
    <div className="flex justify-center">
      <div className="flex flex-col items-center max-w-[1200px] mt-28">
        <div className="">
          <p className="text-purpleText dark:text-lightPurple font-bold text-base lg:text-xl text-center leading-[45px] lg:leading-[60px]">
        ...
        {quote ? (
          <div className="my-12 mx-10">
            <blockquote className="bg-loading-screen-quote gradient-text text-sm lg:text-base font-bold text-center leading-[30px] lg:leading-[40px]">
          ...
          <div className="my-12 mx-10">
            <blockquote className="bg-loading-screen-quote gradient-text lg:text-base font-bold leading-[30px] lg:leading-[40px]">
              "Embrace the journey of growth, for each step forward illuminates
              the path to becoming the best version of yourself." - ChatGPT
            </blockquote>
          </div>
        )}
        <div>
